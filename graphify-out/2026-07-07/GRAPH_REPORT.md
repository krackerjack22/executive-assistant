...
- pdf_inspect.py
- _profile_with_siblings
- _make_synonyms_with_learned
- Calendar Integration Logic
- test_extract_apply.py
...
- run_all_tests.sh
- conftest.py
- test_phone_email_is_low
- test_trust_name_allowed_for_trust_field
- test_trust_name_excluded_for_subscriber_name
- test_pcp_phone_with_section_hint
- test_phone_without_section_hint_returns_patient_phone
- test_source_explanation_present_all_fields
- test_vietnamese_does_not_match_legal_name
- test_city_does_not_match_electricity
- test_name_still_matches_patient_name
- test_street_still_returned_for_address_field
- test_universal_context_rule_applies_to_non_tyler_profile
- test_email_filled_when_contact_email_present
- test_address_not_returned_for_employer_address
- test_alternative_shape
- Executive Assistant README
- date

## God Nodes (most connected - your core abstractions)
1. `_index()` - 61 edges
2. `_tyler()` - 42 edges
3. `map_pdf_field()` - 30 edges
4. `_profile()` - 17 edges
5. `_index()` - 17 edges
6. `_make_result()` - 11 edges
7. `_run_interview_mode()` - 10 edges
8. `TestBuildPrompt` - 9 edges
9. `_load_all_profiles()` - 9 edges
10. `_run_resolve()` - 9 edges
...
- `Web Form Autofill` --shares_data_with--> `Personal Assets Library`  [INFERRED]
  skills/web-form-autofill/SKILL.md → SKILL.md
...
- **Calendar Automation Flow** — skills_add2calendar_references_custom_gpt__calendar_integration, skills_add2calendar_references_custom_gpt__calendar_integration_final_raw_array, skills_add2calendar_references_custom_gpt__calendar_integration_wrapped_data, skills_summer_camp_scheduler_references_calendar_assistant_sop [EXTRACTED 0.90]

## Communities (72 total, 19 thin omitted)

### Community 0 - "field_mapper.py"
Cohesion: 0.06
Nodes (59): date, clear_synonyms_cache(), _determine_confidence(), _is_combined_city_state_zip(), _is_emergency_contact_field(), _is_language_field(), _is_mailing_if_different(), _is_parent_name_field() (+51 more)

### Community 1 - "profile_loader.py"
Cohesion: 0.06
Nodes (46): list_profiles(), _load_all_raws(), load_index(), load_profile(), _load_raw(), _profiles_path(), Path, Load and resolve profile JSON files with inheritance expansion. (+38 more)

### Community 2 - "test_learning.py"
Cohesion: 0.05
...
### Community 3 - "test_acroform_roundtrip.py"
Cohesion: 0.05
...
### Community 4 - "test_formatters.py"
Cohesion: 0.06
Nodes (13): apply_format(), format_date(), format_grade(), format_phone(), format_today(), date, Value formatters applied at fill-time: dates, phones, grades., Convert an ISO-8601 date string (YYYY-MM-DD) to MM/DD/YYYY.      Passes through (+5 more)
...
### Community 7 - "test_qa_reviewer.py"
Cohesion: 0.06
...
### Community 8 - "autofill.py"
Cohesion: 0.12
Nodes (29): _collect_batch(), _dot_path_from_source(), _field_section(), _fill_pdf(), main(), _offer_learning(), Path, Interactive resolution of low-confidence fields, then commit. (+21 more)
...
### Community 13 - "test_overlay.py"
Cohesion: 0.08
Nodes (23): Tests for skills/form-autofill/overlay.py — spatial overlay for flattened PDFs., High-confidence labels from the fixture have non-None mapped values., filled_count + skipped_count == len(fields)., Commit mode writes an output PDF., Commit result includes 'output' key with the written path., Filled PDF contains overlaid profile values., A completely blank PDF (no words, no images) raises a ValueError., An image-only PDF attempts to run OCR workflow and raises if not interactive. (+15 more)
...
### Community 17 - "overlay.py"
Cohesion: 0.15
Nodes (16): Args, _detect_label(), fill(), _find_underscore_line_above(), _find_underscore_line_right(), _group_into_lines(), Path, Spatial overlay fill for flattened (non-AcroForm) PDFs. (+8 more)

### Community 18 - "test_field_mapper.py"
Cohesion: 0.20
...
### Community 20 - "Child Skills Registry"
Cohesion: 0.17
...
### Community 22 - "test_profile_loader.py"
Cohesion: 0.15
...
### Community 23 - "test_profile_writer.py"
Cohesion: 0.37
...
### Community 25 - "_profile_with_siblings"
Cohesion: 0.25
...
### Community 26 - "_make_synonyms_with_learned"
Cohesion: 0.29
...
### Community 27 - "Calendar Integration Logic"
Cohesion: 0.33
Nodes (6): Calendar Integration Logic, Custom GPT Calendar Integration, Custom GPT Calendar Integration (Raw Array), Custom GPT Calendar Integration (Wrapped Data), Calendar Scheduling Assistant SOP, Summer Camps 2026 Selected Schedule

### Community 28 - "test_extract_apply.py"
Cohesion: 0.33
...
### Community 29 - "_tyler_with_vault_ref"
Cohesion: 0.33
...
### Community 31 - "schema_builder.py"
Cohesion: 0.50
...
### Community 32 - "get_spatial_coordinates"
Cohesion: 0.40
Nodes (4): get_spatial_coordinates(), Path, Gemini 1.5 Spatial Bounding Box extraction for PDF forms.  Sends a blank PDF pag, Call Gemini Vision to extract bounding boxes for target fields.      Args:

### Community 33 - "make_flattened_form"
Cohesion: 0.40
...
### Community 34 - "requirements.txt"
Cohesion: 0.50
Nodes (3): browser-use, langchain-google-genai, playwright

## Knowledge Gaps
- **16 isolated node(s):** `run_all_tests.sh script`, `Args`, `Executive Assistant README`, `Google Calendar Integration`, `Calendar Integration Logic` (+11 more)
